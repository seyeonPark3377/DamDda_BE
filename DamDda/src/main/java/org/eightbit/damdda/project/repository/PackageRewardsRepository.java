...
public interface PackageRewardsRepository extends JpaRepository<PackageRewards,Long> {
    ...
    @Query("SELECT DISTINCT pr FROM PackageRewards pr " +
            "JOIN FETCH pr.projectPackage " +
            "JOIN FETCH pr.projectReward " +
            "WHERE pr.project.id = :projectId")
    List<PackageRewards> findAllByProjectIdWithPackageAndRewards(@Param("projectId") Long projectId);
